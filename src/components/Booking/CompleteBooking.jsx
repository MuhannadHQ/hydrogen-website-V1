...
const setCallbackUrl = (orderData) =>
{
...
export const CompleteBooking = ({ paymentMethod, orderData, disablePayment, currentStep, setCurrentStep }) =>
{
  const [status, setStatus] = useState("")

  const callbackUrl = setCallbackUrl(orderData);
  return (
    <div className="bg-secondary rounded-lg shadow py-8 text-gray-500">
      <h3
        ...
        <span
          className={`flex justify-center items-center w-7 h-7 rounded-full border ${currentStep >= 2 ? "border-primary" : "border-gray-500"}`}>3</span>
        إتمام الشراء
      </h3>
      {
        currentStep === 2 &&
          ...
          <>
            ...
            { paymentMethod === "cod" &&
              ...
              { paymentMethod === "installment" &&
                <InstallmentsPlan callbackUrl={callbackUrl} total={orderData.orderTotal}/>
              }
          </>
      }
    </div>
